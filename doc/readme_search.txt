...
3. How to run the program?
   ...
   discussion). The following command should work with a test-file
   provided (for example, from a directory containing the data):

   ./search -d . -o ./candidates -i 42 -b 271 --whitenoise

   The program will proceed assuming white noise. 
 
   Complete list of switches include:
	-a <file>	performs a software injection with the signal parameters 
			from the <file> (details in sigen description)
	-b <num>	Frequency band number
	-c <dir>	changes the working directory to <dir>
	-d <dir>	Input data directory 
	-f <fftinterp>  INT (default; simple interpolation of shorter Fourier 
			transforms using interbinning) 
			or FFT (longer Fourier transforms with interpolation 
			by zero padding)
	-i <num>	Frame number
	-h <hemi>	Hemisphere to search (default is 0 - both) 			
	-l <label>	additional label for the input and output files
	-o <dir>	Output directory 
	-r <file>	performs the search within the grid range from the <file> 
			(4 lines, each containing 2 integer numbers describing the 
			range of spindowns, sky positions and hemispheres in [linear] 
			grid coordinates)
	-t <num>	threshold for the F-statistic (default is 20) 	

	--whitenoise    assumes white Gaussian noise
	--nospindown    perform a search without spindowns
