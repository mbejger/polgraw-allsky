...
)
{
	...
	*/
	...
	shift_time_mod<<<(sett->nfft + BLOCK_SIZE - 1)/BLOCK_SIZE, BLOCK_SIZE>>>(shft1, het0, nSource[0], nSource[1], nSource[2],
	...
	*/
	...
	resample_postfft<<<(sett->Ninterp + BLOCK_SIZE - 1)/BLOCK_SIZE, BLOCK_SIZE>>>(arr->cu_xa, arr->cu_xb, sett->nfft, sett->Ninterp, nyqst);
	CudaCheckError();
	...
	scale_fft<<<(sett->Ninterp + BLOCK_SIZE - 1)/BLOCK_SIZE, BLOCK_SIZE>>>(arr->cu_xa, arr->cu_xb, ft, sett->Ninterp);
	CudaCheckError();
	...
	double_to_float<<<(sett->N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(arr->cu_xar, arr->cu_xbr,
...
//#mb
				cudaMemsetAsync(arr->cu_xa_f+sett->N, 0, (sett->nfft - sett->N)*sizeof(float complex));
				cudaMemsetAsync(arr->cu_xb_f+sett->N, 0, (sett->nfft - sett->N)*sizeof(float complex));

//				pad_zeros<<<(sett->nfft - sett->N + BLOCK_SIZE - 1)/BLOCK_SIZE, BLOCK_SIZE>>>(arr->cu_xa_f+sett->N, arr->cu_xb_f+sett->N, sett->nfft - sett->N);
				CudaCheckError();
...
//#mb
				cudaMemsetAsync(arr->cu_xa_f+sett->N, 0, (sett->nfft*sett->fftpad - sett->N)*sizeof(float complex));
				cudaMemsetAsync(arr->cu_xb_f+sett->N, 0, (sett->nfft*sett->fftpad - sett->N)*sizeof(float complex));

// 				pad_zeros<<<(sett->nfft*sett->fftpad - sett->N + BLOCK_SIZE - 1)/BLOCK_SIZE, BLOCK_SIZE>>>(arr->cu_xa_f+sett->N, arr->cu_xb_f+sett->N, sett->nfft*sett->fftpad - sett->N);
...
void save_candidates(FLOAT_TYPE* cu_cand_buffer, FLOAT_TYPE* cand_buffer, int *cand_buffer_count, const char* outname) {
	CudaSafeCall(cudaMemcpy(cand_buffer, cu_cand_buffer, sizeof(FLOAT_TYPE)*NPAR * (*cand_buffer_count), cudaMemcpyDeviceToHost));
...
//	CudaSafeCall ( cudaMalloc((void**)&cu_mu, sizeof(float)*nav_blocks) );
	...
	//sum fstat in blocks
	reduction_sum<<<blocks, BLOCK_SIZE, BLOCK_SIZE*sizeof(float)>>>(cu_F, cu_mu_t, N);
	CudaCheckError();
