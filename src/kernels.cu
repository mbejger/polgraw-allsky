...
#ifdef COMP_FLOAT
		FLOAT_TYPE c = __cosf(phase); 
		FLOAT_TYPE s = __sinf(phase);
#else
		FLOAT_TYPE c = cos(phase);
		FLOAT_TYPE s = sin(phase);
#endif
...
__global__ void compute_Fstat(COMPLEX_TYPE *xa, 
							  COMPLEX_TYPE *xb,
							  FLOAT_TYPE *F, 
							  FLOAT_TYPE crf0, 
							  int N) { // N = nmax - nmin
	...
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < N) {
//#mb		F[i] = ( xa[i].x*xa[i].x + xa[i].y*xa[i].y + xb[i].x*xb[i].x + xb[i].y*xb[i].y) / (crf0);
		F[i] = __fdividef( xa[i].x*xa[i].x + xa[i].y*xa[i].y + xb[i].x*xb[i].x + xb[i].y*xb[i].y, crf0);
