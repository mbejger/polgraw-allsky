...
/// be careful and do not convert vectors twice time (exp. by using 'convert' function)
std::vector<double> GridS1::grid(double c0, double xi, unsigned int nfft) const
{
    ...
    temp = num::multiply_AB(q0, num::inverse( num::cholesky( m_fm->postrmf(xi), dim), dim), dim, dim, dim);
...
std::vector<double> GridS1::convert(double c0, double xi, const std::vector<double>& grid_prim)
{
    ...
    temp = num::multiply_AB(grid_prim, num::inverse( num::cholesky( m_fm->postrmf(xi), dim), dim), dim, dim, dim);
