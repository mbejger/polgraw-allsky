...
int main(int argc, char **argv) { 
  ...
  fread(&w, sizeof(unsigned short int), 1, data); 
  
  fread(&allfra, sizeof(int), 4*w, data);
 
  printf("# Number of all frames: %hu\n", w); 
  printf("# Frames information - frame number, number of all and unique candidates in the trigger file, band number:\n"); 

  for(i=0; i<w; i++) printf("%d %d %d %d ", allfra[4*i], allfra[4*i+1], allfra[4*i+2], allfra[4*i+3]);
