...
int main (int argc, char *argv[]) {
     ...
     int N, lfft, lfftr, lfftm, lenx, ldat, n, i, j, indx, yndx, notempty, \
	  ...
	  nxRest = 0, nxall = 0, flidx, offset, *segar=NULL, gen_sci;
	  ...
	  *tptr, *flnames[MAX_FILES], *segments_fname;
...
#endif
	  if(gen_sci){
	       printf("gpsdRest = %f,  mingps =%f, gpsd = %f, gpsdn = %f", gpsdRest, mingps, gpsd, gpsdn);
	       int offsetgps=(int)round((gpsd - mingps)/dt);
	       printf("offsetgps = %d, nxall = %d numbersegan = %ld\n", offsetgps, nxall, numbersegan);
